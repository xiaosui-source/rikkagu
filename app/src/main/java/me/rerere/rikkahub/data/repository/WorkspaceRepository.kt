...
class WorkspaceRepository(
    ...
    suspend fun unpackApk(
        ...
        // 可选：frida（需要root真机时用）
        executeCommand(id, "pip3 install -q frida-tools 2>/dev/null || true", timeoutMillis = 600_000)
        ...
        val cmd = buildString {
            ...
            appendLine("PYEOF")
            appendLine("echo '=== 5. 生成脱壳脚本（免root优先）==='")
            ...
            appendLine("UNIEOF")
            appendLine("echo '=== 5b. 生成 Frida 脚本（备选，需root真机）==='")
            ...
            appendLine("echo '· 备选(root真机)：frida 跑 frida_dump.py <包名>，或 BlackDex'")
        }
        return executeCommand(id, cmd, timeoutMillis = 1_800_000)
    }
