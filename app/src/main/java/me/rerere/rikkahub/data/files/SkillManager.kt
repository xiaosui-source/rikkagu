...
5. **循环**：重复看盘→算棋→落子→验证，直到棋局结束。每轮告诉用户你下了什么、局面如何。整个循环（截图+计算+落子）每手必须在 30 秒内完成：识别快、算棋狠、落子果断，不磨蹭。
6. **前提**：需用户已开启灵犀的无障碍服务（若工具返回 accessibility 未启用错误，提示用户去 设置→无障碍 开启后继续）。
   ⚠ 若你发现自己没有 take_screenshot/tap/find_node 这些工具（调用报"工具不存在"或列表里没有），说明当前助手的「屏幕自动化」未开启：告诉用户去 助手设置 → 本地工具 → 打开「屏幕自动化」，同时开启系统无障碍权限，然后重新发"帮我下"即可。在工具可用前，先降级用聊天内画棋盘坐标对局模式陪用户下。
