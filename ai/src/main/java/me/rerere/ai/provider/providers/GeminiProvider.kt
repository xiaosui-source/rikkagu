...
import me.rerere.rikkahub.data.model.modelModelOption
import me.rerere.rikkahub.data.model.modelModelParameter
import me.rerere.rikkahub.data.model.modelToolPrompt
import me.rerere.rikkahub.data.model.modelParameterCategory
import me.rerere.rikkahub.data.statsProviderUsageNormalizer
import me.rerere.ai.util.ChatUtils
import me.rerere.ai.util.ChatMarkupRegex
import me.rerere.ai.util.HttpLogSanitizer
import me.rerere.ai.util.StreamingJsonXmlConverter
import me.rerere.ai.util.TokenCacheManager
import me.rerere.ai.util.exceptionsUserCancellationException
import me.rerere.ai.util.streamMutableSharedStream
import me.rerere.ai.util.streamStream
import me.rerere.ai.util.streamStreamCollector
import me.rerere.ai.util.streamTextStreamEvent
import me.rerere.ai.util.streamTextStreamEventType
import me.rerere.ai.util.streamwithEventChannel
import me.rerere.ai.util.streamstream
...
import me.rerere.ai.provider.chatllmprovider
...
open class GeminiProvider(
    ...
    override fun cancelStreaming() {
        ...
        activeResponse?.let {
            try {
                it.close()
                android.util.Log.d(TAG, "已强制关闭Response流")
            } catch (e: Exception) {
                android.util.Log.w(TAG, "关闭Response时出错: ${e.message}")
            }
        }
        activeResponse = null
        ...
        activeCall?.let {
            if (!it.isCanceled()) {
                it.cancel()
                android.util.Log.d(TAG, "已取消当前流式传输，Call已中断")
            }
        }
        activeCall = null

        android.util.Log.d(TAG, "取消标志已设置，流读取将立即被中断")
    }
    ...
    private fun parseXmlToolCalls(content: String): GeminiFunctionCallPayload {
        ...
        val functionCalls =
            matches.map { match ->
                ...
                android.util.Log.d(TAG, "XML→GeminiFunctionCall: $toolName")
                JSONObject().apply {
                    put("name", toolName)
                    put("args", args)
                }
    ...
    private fun parseXmlToolResults(content: String): Pair<String, List<JSONObject>?> {
        ...
        matches.forEach { match ->
            ...
            android.util.Log.d(TAG, "解析Gemini functionResponse: $toolName, content length=${resultContent.length}")
    ...
    private fun logLargeString(tag: String, message: String, prefix: String = "") {
        ...
        if (message.length > maxLogSize) {
            ...
            for (i in 0 until chunkCount) {
                ...
                // 打印带有编号的日志
                android.util.Log.d(tag, "$prefix Part ${i+1}/$chunkCount: $chunkMessage")
            }
        } else {
            // 消息长度在限制之内，直接打印
    ...
    private fun logFinalOutput(content: CharSequence, prefix: String = "Gemini final output: ") {
        ...
        if (finalOutput.isBlank()) {
            android.util.Log.d(TAG, "${prefix.trimEnd()}[empty]")
            return
        }
    ...
    private suspend fun handleRetryableError(
        ...
        android.util.Log.w(TAG, "$errorText，将在 ${retryDelayMs}ms 后进行第 $newRetryCount 次重试...", exception)
    ...
    override suspend fun sendMessage(
        ...
        android.util.Log.d(TAG, "发送消息到Gemini API, 模型: $modelName")
            ...
            try {
                ...
                withContext(kotlinx.coroutines.Dispatchers.IO) {
                    ...
                    try {
                        ...
                        android.util.Log.d(TAG, "收到初始响应, 耗时: ${duration}ms, 状态码: ${response.code}")
    ...
    protected open suspend fun createRequest(
        ...
        android.util.Log.d(TAG, "请求URL: $requestUrl")
    ...
    private suspend fun processStreamingResponse(
        ...
        android.util.Log.d(TAG, "开始处理响应流")
        ...
        try {
            ...
            android.util.Log.d(TAG, "响应处理完成: 共${lineCount}行, ${jsonCount}个JSON块, 提取${contentCount}个内容块")

            // 检查是否还有未解析完的JSON
            if (isCollectingJson && completeJsonBuilder.isNotEmpty()) {
                try {
                    val finalJson = completeJsonBuilder.toString()
                    android.util.Log.d(TAG, "处理最终收集的JSON，长度: ${finalJson.length}")
    ...
    private suspend fun processNonStreamingResponse(
        ...
        android.util.Log.d(TAG, "开始处理非流式响应")
        val responseBody = response.body ?: throw IOException(context.getString(R.string.gemini_response_empty))
