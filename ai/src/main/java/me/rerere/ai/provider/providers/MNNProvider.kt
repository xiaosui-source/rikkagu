...
import me.rerere.core.chathooks
import me.rerere.ai.util.FFmpegUtil
import me.rerere.ai.util.ImagePoolManager
import me.rerere.ai.util.MediaPoolManager
...
import me.rerere.rikkahub.data.model.modelParameterValueType
...
class MNNProvider(
    ...
    override fun cancelStreaming() {
        ...
        android.util.Log.d(TAG, "已取消MNN推理（已通知底层中断）")
    }
    ...
    private fun logLargeString(prefix: String, message: String) {
        ...
        for (index in 0 until chunkCount) {
            ...
            android.util.Log.d(TAG, "$prefix Part ${index + 1}/$chunkCount: $chunk")
        }
    }
    ...
    private suspend fun initModel(): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            if (llmSession == null) {
                android.util.Log.d(TAG, "初始化MNN LLM模型: $modelName")
                
                // 获取模型目录
                val modelDir = getModelDir(context, modelName)
                android.util.Log.d(TAG, "模型目录: $modelDir")
                ...
                val backendType = when (forwardType) {
                    ...
                    7 -> "vulkan"
                        ...
                        android.util.Log.w(TAG, "未知的 forwardType: $forwardType，使用默认 CPU")
                        "cpu"
                    }
                }
                
                android.util.Log.d(TAG, "创建MNN LLM会话，后端: $backendType, 线程数: $threadCount")
                ...
                android.util.Log.d(TAG, "内存模式: $memoryMode (后端: $backendType)")
                
                // 创建缓存目录（用于存放 mnn_cachefile.bin 等临时文件）
                val cacheDir = File(context.cacheDir, "mnn_cache")
                if (!cacheDir.exists()) {
                    val created = cacheDir.mkdirs()
                    android.util.Log.d(TAG, "创建MNN缓存目录: $created")
                }
                android.util.Log.d(TAG, "MNN缓存目录: ${cacheDir.absolutePath}")
                android.util.Log.d(TAG, "缓存目录存在: ${cacheDir.exists()}, 可写: ${cacheDir.canWrite()}")
            ...
            android.util.Log.e(TAG, "初始化MNN LLM模型失败", e)
            Result.failure(e)
        }
    }
    ...
    private suspend fun countTokens(text: String): Int = withContext(Dispatchers.IO) {
        try {
            ...
            android.util.Log.w(TAG, "Token计数失败，使用估算", e)
            estimateTokens(text)
        }
    }
    ...
    private fun writeBase64ToTempFile(base64: String, mimeType: String, prefix: String): File? {
        ...
        return try {
            ...
            android.util.Log.e(TAG, "写入多模态临时文件失败: mimeType=$mimeType", e)
    ...
    override suspend fun sendMessage(
        ...
        try {
            ...
            if (chatHistory.isEmpty()) {
                android.util.Log.d(TAG, "消息为空，跳过处理")
                return@stream
            }
            ...
            android.util.Log.e(TAG, "发送消息时出错", e)
            throw e
        } finally {
    ...
    override suspend fun testConnection(context: Context): Result<String> = withContext(Dispatchers.IO) {
        try {
            ...
            android.util.Log.e(TAG, "测试连接失败", e)
            Result.failure(e)
        }
    }
    ...
    private fun applyModelParameters(session: MNNLlmSession, parameters: List<ModelParameter<*>>) {
        try {
                            ...
                            when (param.valueType) {
                                ...
                                ParameterValueType.OBJECT -> {
                                    ...
                                    val parsed: Any? = try {
                                        ...
                                        android.util.Log.w(TAG, "自定义OBJECT参数解析失败: ${param.apiName}", e)
                                        null
                                    }
                                    if (parsed != null) {
            ...
            if (configMap.isNotEmpty()) {
                ...
                android.util.Log.d(TAG, "应用模型参数: $configJson")
                val success = session.setConfig(configJson)
                if (!success) {
                    android.util.Log.w(TAG, "部分模型参数设置失败")
                }
            } else {
                android.util.Log.d(TAG, "没有启用的模型参数需要应用")
            }
        } catch (e: Exception) {
            android.util.Log.e(TAG, "应用模型参数时出错", e)
        }
    }
    ...
    override fun release() {
        try {
            llmSession?.release()
            llmSession = null
            android.util.Log.d(TAG, "MNN LLM资源已释放")
        } catch (e: Exception) {
            android.util.Log.e(TAG, "释放资源时出错", e)
        }
    }
}
